...
interface Props {
  ...
  // We can pass the auth logic purely as auth submission or keep auth state here.
  // We will keep auth form here and call an async prop.
...
export default function LandingPage({
  ...
  const AuthFormContent = () => (
    <form onSubmit={submitAuth} className="bg-white/5 p-8 sm:p-10 rounded-[2rem] border border-white/10 backdrop-blur-xl shadow-2xl relative">
      ...
      <div className="space-y-4 mb-8">
        <div>
          <input
            ...
            required
            className="w-full bg-black/40 border border-white/10 rounded-2xl px-5 py-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500 transition-all font-medium"
          />
        </div>
        <div>
          <input
  ...
  if (showTest) {
    return (
      <div className="min-h-screen bg-nudge-primary flex flex-col items-center justify-center p-4">
  ...
  if (testResult) {
    ...
    return (
      <div className="min-h-screen bg-nudge-primary text-white overflow-y-auto w-full relative pb-24 scroll-smooth">
        {/* Background elements */}
        ...
        <div className="max-w-6xl mx-auto px-4 pt-12 relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
            ...
            <motion.div 
              ...
              className="space-y-6"
            >
               <div className="bg-white/5 border border-white/10 backdrop-blur-sm p-8 rounded-3xl relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-8 opacity-5">
                     <TrendingUp className="w-48 h-48" />
                  </div>
                  <h3 className="text-2xl font-bold mb-8">Expert Insights</h3>
                  ...
                  <div className="space-y-8 relative z-10">
                    <div className="flex gap-5">
                       ...
                       <div>
                          <h4 className="text-lg font-semibold text-white">Savings Trajectory</h4>
                          <p className="text-gray-400 text-sm mt-1 leading-relaxed">Based on your stated income and expenses, your estimated savings rate is {Math.max(0, Number(estimatedSavingsRate))}%. Our AI can help you increase this painlessly.</p>
                       </div>
                    </div>
                    <div className="flex gap-5">
                       ...
                       <div>
                          <h4 className="text-lg font-semibold text-white">Credit Optimization</h4>
                          <p className="text-gray-400 text-sm mt-1 leading-relaxed">With a score of {testResult.credit_score}, you are positioned to {testResult.credit_score >= 700 ? 'maximize premium credit rewards' : 'build credit up systematically'} using targeted AI strategies.</p>
                       </div>
                    </div>
                    <div className="flex gap-5">
                       ...
                       <div>
                          <h4 className="text-lg font-semibold text-white">Risk Profile ({testResult.risk_tolerance.charAt(0).toUpperCase() + testResult.risk_tolerance.slice(1)})</h4>
                          <p className="text-gray-400 text-sm mt-1 leading-relaxed">Our AI will structure a tailored {testResult.risk_tolerance} plan to pursue your primary goal of {testResult.primary_goal.replace('-', ' ')} while limiting market exposure.</p>
                       </div>
                    </div>
                  </div>
               </div>

               <div className="bg-gradient-to-br from-accent-500/10 to-transparent border border-accent-500/20 backdrop-blur-sm p-6 md:p-8 rounded-3xl flex items-center gap-5">
                  ...
                  <p className="text-gray-300 font-medium text-sm md:text-base leading-relaxed">Create your free account now to get day-by-day actions exactly like these, customized specifically for your portfolio.</p>
               </div>
            </motion.div>
  ...
  return (
    <div className="min-h-screen bg-nudge-primary text-nudge-primary overflow-x-hidden relative scroll-smooth">
      {/* Background elements */}
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-accent-500/10 blur-[120px] rounded-full pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-purple-500/10 blur-[120px] rounded-full pointer-events-none" />

      {/* Nav */}
      <nav className="fixed top-0 left-0 right-0 p-6 z-50 bg-nudge-primary/80 backdrop-blur-lg border-b border-white/5">
      ...
      {/* Hero */}
      <BackgroundPaths title="Master your money with zero effort.">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.5 }}>
          ...
          <p className="text-xl text-nudge-secondary max-w-2xl mx-auto mb-10 leading-relaxed font-medium">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <button 
              ...
              className="px-8 py-4 rounded-full bg-white/5 border border-white/10 text-nudge-primary font-bold text-lg hover:bg-white/10 hover:-translate-y-0.5 transition-all duration-300 shadow-lg hover:shadow-xl w-full sm:w-auto justify-center"
            >
              Sign Up directly
            </button>
          </div>
      ...
      {/* Features Grid */}
      <section className="py-20 px-4 relative z-10 bg-black/20 border-y border-white/5">
          ...
          <AnimatePresence mode="popLayout">
            {testResult && (
              <motion.div 
                ...
                className="mb-8 p-6 bg-accent-500/10 border border-accent-500/30 rounded-3xl"
              >
                <div className="flex items-center gap-3 mb-2">
                  <Save className="text-accent-400 w-6 h-6" />
                  <h3 className="text-xl font-bold text-white">Save your results</h3>
                </div>
                <p className="text-accent-200/80 text-sm mb-4">You completed the financial test! Create a free account now to save your custom profile and start optimizing.</p>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      {/* Footer */}
      <footer className="py-8 text-center text-gray-500 border-t border-white/5 bg-black/40 z-10 relative">
